	...
	t.Run("method=authenticate", func(t *testing.T) {
		for k, tc := range []struct {
			...
			},
			{
				...
				r:      &http.Request{Host: "some-host", Header: http.Header{"Authorization": {"bearer token"}}, Method: "POST"},
				config: []byte(`{"preserve_host": true, "headers": {"X-Test": "test123", "X-Forwarded-For": "some-other-host", "Z-Test": "test987"}}`),
				setup: func(t *testing.T, m *httprouter.Router) {
					m.POST("/oauth2/introspect", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
						require.NoError(t, r.ParseForm())
						require.Equal(t, "token", r.Form.Get("token"))
						require.Equal(t, "some-host", r.Header.Get("X-Forwarded-Host"), "preserve_host takes precedence over headers")
						require.Equal(t, "test123", r.Header.Get("X-Test"), "value configured in headers is set")
						require.Equal(t, "test987", r.Header.Get("Z-Test"), "value configured in headers is set")
						require.NoError(t, json.NewEncoder(w).Encode(&AuthenticatorOAuth2IntrospectionResult{
							Active:   true,
							Subject:  "subject",
							Audience: []string{"audience"},
